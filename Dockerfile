...
# 1. Install standard machine learning requirements
RUN pip install --no-cache-dir diffusers transformers accelerate

# 2. Directly clone the node into the image's pre-configured custom_nodes directory
# This avoids comfy-cli initialization issues entirely.
